...
#define TESSELATE_H
	...
	public:
		...
		void tesselate(const double* p1, const double* p2, const double* p3, uint32_t recurse)
		{
			...
			if (recurse < mMaxDepth)
			{
				l1 = fm_distanceSquared(pp1, pp2);
				l2 = fm_distanceSquared(pp2, pp3);
				l3 = fm_distanceSquared(pp3, pp1);
...
void test(uint32_t index, const REAL* vertices, uint32_t vstride)
{
	const REAL* tmpResult = FLOAT_MATH::computePos(index, vertices, vstride);
	std::cout << *tmpResult << '\n';
}
...
int main()
{
	int index = 12;
	const REAL tmp = 89;
	const REAL* vertices = &tmp;
	uint32_t vstride = 2;

	test(index, vertices, vstride);
